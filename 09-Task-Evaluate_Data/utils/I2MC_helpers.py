...
def get_fixations(final_weights, timestamp, xpos, ypos, missing, par):
    ...
    cutoff = np.nanmean(final_weights) + cutoffstd * \
        np.nanstd(final_weights, ddof=1)
    ...
    for p in range(len(starttime)):
        if missing[fixstart[p]]:
            fixstart[p] = fixstart[p] + \
                np.argmax(np.invert(missing[fixstart[p]:fixend[p]+1]))
            starttime[p] = timestamp[fixstart[p]]
        if missing[fixend[p]]:
            fixend[p] = fixend[p] - \
                (np.argmax(
                    np.invert(missing[fixstart[p]:fixend[p]+1][::-1]))+1)
    ...
    nextSamp = np.min(np.vstack(
        [fixend+1, np.zeros(len(fixend), dtype=int)+len(timestamp)-1]), axis=0)
    ...
    # first check if there are fixations in the first place, or we'll index into non-existing data
    if not len(fixend) == 0 and fixend[-1] == len(timestamp):
    ...
    # vector for whether fixation is flanked by data loss
    flankdataloss = np.zeros(fixstart.shape)
    ...
    for a in range(len(fixstart)):
        ...
        flankdataloss[a] = (fixstart[a] > 0 and missing[fixstart[a]-1]
                            ) or (fixend[a] < len(xpos)-1 and missing[fixend[a]+1])

        # fraction of data loss during fixation that has been (does not count
        # data that is still lost)
        fracinterped[a] = np.sum(
            np.invert(np.isnan(xposf[qMiss])))/(fixend[a]-fixstart[a]+1)
...
def get_fix_stats(xpos, ypos, missing, fix, pix_per_deg=None):
    ...
    for a in range(len(fstart)):
        ...
        if np.sum(qMiss) == len(qMiss):
            ...
            rangeX[a] = (np.max(xposf[np.invert(qMiss)]) -
                         np.min(xposf[np.invert(qMiss)]))
            rangeY[a] = (np.max(yposf[np.invert(qMiss)]) -
                         np.min(yposf[np.invert(qMiss)]))
...
def I2MC(gazeData, options=None, logging=True, logging_offset=""):
    ...
    check_fun('missingx', opt,
              'value indicating data loss for horizontal position')
    ...
    # screen size (e.g. in cm). Optional, specify if want fixation statistics in deg
    par['scrSz'] = opt.pop('scrSz', None)
    # screen distance (in same unit as size). Optional, specify if want fixation statistics in deg
    par['disttoscreen'] = opt.pop('disttoscreen', None)

    # parameters with defaults:
    # CUBIC SPLINE INTERPOLATION
    # max duration (s) of missing values for interpolation to occur
    par['windowtimeInterp'] = opt.pop('windowtimeInterp', .1)
    # amount of data (number of samples) at edges needed for interpolation
    par['edgeSampInterp'] = opt.pop('edgeSampInterp', 2)
    # maximum displacement during missing for interpolation to be possible. Default set below if needed
    par['maxdisp'] = opt.pop('maxdisp', None)
    ...
    # use chebychev filter when downsampling? True: yes, False: no. requires signal processing toolbox. is what matlab's downsampling functions do, but could cause trouble (ringing) with the hard edges in eye-movement data
    par['downsampFilter'] = opt.pop('downsampFilter', True)
    # order of cheby1 Chebychev downsampling filter, default is normally ok, as long as there are 25 or more samples in the window (you may have less if your data is of low sampling rate or your window is small
    ...
    # number of MAD away from median fixation duration. Will be used to walk forward at fixation starts and backward at fixation ends to refine their placement and stop algorithm from eating into saccades
    par['onoffsetThresh'] = opt.pop('onoffsetThresh', 3.)
    # maximum Euclidean distance in pixels between fixations for merging
    par['maxMergeDist'] = opt.pop('maxMergeDist', 30.)
    # maximum time in ms between fixations for merging
    par['maxMergeTime'] = opt.pop('maxMergeTime', 30.)
    # minimum fixation duration (ms) after merging, fixations with shorter duration are removed from output
    par['minFixDur'] = opt.pop('minFixDur', 40.)

    par['first_cluster_fixed_value'] = opt.pop(
        'first_cluster_fixed_value', None)          # TODO
    par['second_cluster_fixed_value'] = opt.pop(
        'second_cluster_fixed_value', None)         # TODO
    ...
    if not par['skip_inputhandling']:
        for key, value in par.items():
            if key in ['xres', 'yres', 'freq', 'missingx', 'missingy', 'windowtimeInterp', 'maxdisp', 'windowtime',
                ...
                # may be None (its an optional parameter)
    ...
    assert np.sum(par['freq'] % np.array(par['downsamples'])
                  ) == 0, 'I2MCfunc: Some of your downsample levels are not divisors of your sampling frequency. Change the option "downsamples"'

    # setup visual angle conversion
    pix_per_deg = None
    if par['scrSz'] is not None and par['disttoscreen'] is not None:
        pix_per_deg = angle_to_pixels(1, par['disttoscreen'], par['scrSz'][0], [
                                      par['xres'], par['yres']])
    ...
    if 'L_X' in data.keys() and 'R_X' not in data.keys():
        ...
        if q2Eyes:
            ...
            llmiss, rrmiss, bothmiss = get_missing(
                data['L_X'], data['R_X'], par['missingx'], data['L_Y'], data['R_Y'], par['missingy'])
            data['left_missing'] = llmiss
            data['right_missing'] = rrmiss

    else:  # we have left and right, average them
        data['average_X'], data['average_Y'], missing, llmiss, rrmiss = average_eyes(
            data['L_X'].array, data['R_X'].array, par['missingx'], data['L_Y'].array, data['R_Y'].array, par['missingy'])
        xpos = data['average_X'].array
        ypos = data['average_Y'].array
        data['average_missing'] = missing
        data['left_missing'] = llmiss
    ...
    xpos, ypos, missingn = windowed_interpolate(
        xpos, ypos, missing, missStart, missEnd, par['edgeSampInterp'])
    if q2Eyes:
    ...
    if not q2Eyes:
        ...
        data['finalweights'], stopped = two_cluster_weighting(xpos, ypos, missingn, par['downsamples'],
                                                              ...
                                                              par['first_cluster_fixed_value'],
                                                              par['second_cluster_fixed_value'])

        # check whether clustering succeeded
        if stopped:
            warnings.warn(
                'I2MC: Clustering stopped after exceeding max errors, continuing to next file \n')
    ...
    # CALCULATE 2-MEANS CLUSTERING FOR SEPARATE EYES
        ...
        with warnings.catch_warnings():
            ...
            data['finalweights'] = np.nanmean(
                np.vstack([finalweights_left, finalweights_right]), axis=0)
    ...
    if logging:
        print(logging_offset +
              'I2MC: Determining fixations based on clustering weight mean for averaged signal and separate eyes + {:.2f}*std'.format(par['cutoffstd']))
    fix = get_fixations(data['finalweights'].array,
                        data['time'].array, xpos, ypos, missing, par)
